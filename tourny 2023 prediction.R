...
games_2023 <- read.csv("2024 and before NCAA Tournament Games.csv",stringsAsFactors = FALSE, strip.white = TRUE)
#for (i in 1:length(players_2023[[1]])) {
new_df_names <- c(names(games_2023), "All.BPM", "WS.40", "PER", "Top.Off.Top.Def.BPM", "Top.Off.Xtreme.Def.BPM")
working_df <- games_2023
...
#for (i in 1:length(games_2023[[1]])) {
for (i in gamesLookedAt) {
  # html_base_beggining <- "https://www.sports-reference.com/cbb/schools/"
  # html_base_ending <- ".html"
  csv_base_ending <- ".csv"
  year <- games_2023$Year[i]
  betterTeam <- tolower(games_2023$Team...Better.Seed[i])
  worseTeam <- tolower(games_2023$Team...Worse.Seed[i])
  ...
  # KP <- c(KP, games_2023$Better.Seed.KP.Expected.Margin[i], games_2023$Worse.Seed.KP.Expected.Marg.[i])
  ...
  seed <- c(seed, games_2023$Better.Seed[i], games_2023$Worse.Seed[i])
}
...
write.csv(analyze.2018, file = "analyze2024withInj.csv")
